...
class OrderTransactionManager(models.Manager):
    def create_new(self, order, amount, success=None, transaction_status=None):
        ...
        log("OrderTransactionManager post order_hash: ",order_hash )
        email_hash = str(order.email).split("@")[0]
        log("OrderTransactionManager post email_hash: ",email_hash )
        final_hash = hashlib.sha1((order_hash + email_hash).encode('utf-8')).hexdigest()[:10]
        log("OrderTransactionManager post final_hash: ",final_hash )
        merchant_order_id = "%s" %(final_hash)
        log("OrderTransactionManager post merchant_order_id: ",merchant_order_id )
        ...
        log("OrderTransactionManager post order: ",order )
        log("OrderTransactionManager post amount: ",amount )
        log("OrderTransactionManager post transaction: ",transaction )
        log("OrderTransactionManager post transaction.merchant_order_id: ",transaction.merchant_order_id )
        log("OrderTransactionManager post transaction.order: ",transaction.order )
        log("OrderTransactionManager post transaction.transaction_id: ",transaction.transaction_id )
        log("OrderTransactionManager post transaction.transaction_id: ",transaction.transaction_id )
...
def order_payment_validation(sender, instance, created, *arg, **kwargs):
