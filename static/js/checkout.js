$(function (){
    var IMP = window.IMP;
    IMP.init('imp06460093'); //가맹정 식별코드 등록
    $('.order-form').on('submit', function (e) {
      ...
      console.log('static func e:  '+e);
      console.log('static func order_id:  '+order_id);
      console.log('static func amount:  '+amount);
      console.log('static func type:  '+type);
      // 결제 정보 생성
      var merchant_id = AjaxStoreTransaction(e, order_id, amount, type);

      console.log('static func merchant_id:  '+merchant_id);

      // 결제 정보가 만들어 졌으면 iamport로 실제 결제 시도
      if (merchant_id !== ''){
        IMP.request_pay({
          merchant_uid: merchant_id,
          name: '결제 테스트: E-Shop product',
...
function AjaxStoreTransaction(e, order_id, amount, type) {
  ...
  var merchant_id = '';
  console.log('AjaxStoreTransaction merchant_id:  '+merchant_id);
  var request = $.ajax({
    method: "POST",
  ...
  console.log('AjaxStoreTransaction request:  '+request);
